defmodule Mix.Tasks.MaterialIcons.Update do
  # Updates the icon set via download from github.
  @moduledoc false
  @shortdoc false

  @vsn "0.14.2"
  @tmp_dir_name "material-icons-elixir"
  ...
  def run(_args) do
    ...
    svgs_dir = Path.join([tmp_dir, "material-design-icons-#{version}", "svg"])

    File.rm_rf!(tmp_dir)
    File.mkdir_p!(tmp_dir)

    url = "https://github.com/marella/material-design-icons/archive/refs/tags/v#{version}.zip"
    ...
    copy_svg_files(Path.join([svgs_dir, "filled"]), "filled")
    copy_svg_files(Path.join([svgs_dir, "outlined"]), "outlined")
    copy_svg_files(Path.join([svgs_dir, "round"]), "round")
    copy_svg_files(Path.join([svgs_dir, "sharp"]), "sharp")
    copy_svg_files(Path.join([svgs_dir, "two-tone"]), "two-tone")
  end
  ...
  defp fetch_body!(url) do
    ...
    Logger.debug("Downloading material icons from #{url}")
